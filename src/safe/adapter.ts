...
export class SafeProviderAdapter implements EthereumProvider {
  ...
  async request(args: RequestArguments): Promise<unknown> {
    if (
      ...
      args.params &&
      (args.params as any)[0].from?.toLowerCase() === this.safe.toLowerCase()
    ) {
      const tx = (args.params as any)[0];
      let operation = 0;
      if (!tx.to) {
        tx.to = this.createLibAddress;
        tx.data = this.createLibInterface.encodeFunctionData('performCreate', [
          tx.value || 0,
          tx.data,
        ]);
        tx.value = 0;
      ...
      const safeTxHash = _TypedDataEncoder.hash(
    ...
    if (args.method === 'eth_getTransactionByHash') {
      const txHash = (args.params as any)[0];
    ...
    if (args.method === 'eth_getTransactionReceipt') {
      ...
      if (safeTx?.transactionHash) {
        const resp = await this.wrapped.request({
          method: 'eth_getTransactionReceipt',
          params: [safeTx.transactionHash],
        });
        if (!resp) return resp;
        const success = resp.logs.find(
          (log: any) =>
            log.topics[0] ===
