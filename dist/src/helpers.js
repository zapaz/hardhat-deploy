...
Note that in this case, the contract deployment will not behave the same if deployed without proxy.
    ...
    async function _deployViaEIP173Proxy(name, options) {
        /* eslint-disable prefer-const */
        let { oldDeployment, updateMethod, updateArgs, upgradeIndex, viaAdminContract, proxyAdminDeployed, proxyAdminName, proxyAdminContract, owner, proxyAdmin, currentProxyAdminOwner, artifact, implementationArgs, implementationName, implementationOptions, proxyName, proxyContract, mergedABI, } = await _getProxyInfo(name, options);
        /* eslint-enable prefer-const */
